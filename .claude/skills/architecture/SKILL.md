...
| `src/game.js`           | `createGame({ rng, map })` + séquence : `drawCards`, `playCard`, `moveUnit`, `attackUnit`, `takeGroundHex`, `takeGround`, `canBreakthrough`, `finishUnit`, `endPlayerTurn`, `endAxisTurn`, `orderableUnits`, `shuffle` + cartes actions : `barrageTargets`, `resolveBarrage`, `validAirTarget`, `resolveAirStrike`, `medicTargets`, `resolveMedics` | tous les modules `src/`                                |
...
| `render/stage.js`       | Canvas : DPR, rendu à la demande (rAF coalescé), scène dynamique, `drawCounter`, explosions `boom(hexes)` auto-animées                                                                                                 | `gfx`, `html`, `src/cards`, `src/sectors`              |
...
`breakthrough`, `action`) vit dans `render/uiState.js`, **pas** dans `state` : purement
visuel ou dérivable de `src/`.
