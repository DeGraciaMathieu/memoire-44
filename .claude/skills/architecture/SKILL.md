...
| `render/stage.js`       | Canvas : DPR, rendu à la demande (rAF coalescé), scène dynamique, `drawCounter`, obstacles (pastille `OBSTACLE_BADGE` au coin du pion qui en occupe un), explosions `boom(hexes)` et glissements `slideUnit(unit, from)` auto-animés, surbrillances du tour IA (`ui.aiFocus` / `ui.aiTargets`)                                                                                                                                                             | `gfx`, `html`, `src/cards`, `src/sectors`, `src/movement` |
