...
| `src/rng.js`            | `mulberry32(seed)` — RNG déterministe injecté par les tests et le mode en ligne                                                                                                                                                                                                                                                                                                        | —                                                         |
| `src/online.js`         | Mode en ligne : `applyRemote(state, msg)` rejoue chez soi l'action du joueur distant (protocole documenté en tête de fichier) — lockstep : même seed + mêmes appels = mêmes jets                                                                                                                                                                                                       | `game`                                                    |
...
| `render/home.js`        | Page d'accueil (`index.html`) : liste les cartes du manifeste `maps/index.json` avec un aperçu du plateau (raster réduit + pastilles d'unités), le choix du camp et du mode (solo / en ligne : `?online=1`, rejoindre via `?join=<code>`), liens vers `game.html?map=<fichier>&side=<camp>`                                                                                            | `src/map`, `board`, `gfx`, `html`                         |
| `render/net.js`         | Client réseau du mode en ligne : `createRoom` / `joinRoom` / `connectRoom` (fetch + EventSource vers `server.js`) — messages opaques, aucune règle                                                                                                                                                                                                                                     | —                                                         |
...
| `server.js` (racine)    | Relais du mode en ligne (`npm run online`) : statique + salons à deux joueurs en SSE (`Last-Event-ID` rejoué) — Node natif, zéro dépendance, aucune règle de jeu                                                                                                                                                                                                                        | —                                                         |
...
- Mode en ligne (`render/app.js`) : l'hôte crée le salon (`?online=1`, seed + carte +
  camp), l'invité le rejoint (`?join=<code>`). Chaque action locale légale est envoyée
  (`send`) après application ; les messages du pair sont mis en file et rejoués un à un
  (`playRemoteMsg`) avec le tempo du tour de l'IA. La fin de tour passe par
  `endTurn(state, side)` (neutre) des deux côtés ; l'IA ne joue jamais en ligne.
...
| Séquence de tour, nouvelle action de jeu          | `src/game.js` + test (+ événement bus) — et répercuter le protocole dans `src/online.js` si l'action est jouable en ligne |
| Protocole du mode en ligne (nouveau message)      | `src/online.js` + test (+ envoi dans `render/app.js`) |
| Relais / salons du mode en ligne                  | `server.js` + test                     |
| Transport réseau côté navigateur                  | `render/net.js`                        |
