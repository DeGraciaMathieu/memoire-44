...
| `src/game.js`           | `createGame({ rng, map })` + séquence : `drawCards`, `playCard`, `moveUnit`, `attackUnit`, `takeGroundHex`, `takeGround`, `canBreakthrough`, `canCutWire`, `cutWire`, `finishUnit`, `endPlayerTurn`, `endAxisTurn`, `orderableUnits`, `shuffle` + cartes actions : `barrageTargets`, `resolveBarrage`, `validAirTarget`, `resolveAirStrike`, `medicTargets`, `resolveMedics` | tous les modules `src/`                                   |
...
| `render/home.js`        | Page d'accueil (`index.html`) : liste les cartes du manifeste `maps/index.json` avec un aperçu du plateau (raster réduit + pastilles d'unités), liens vers `game.html?map=<fichier>`                                                                                                                                                                                         | `src/map`, `board`, `gfx`, `html`                         |
