...
| `render/stage.js`       | Canvas : DPR, rendu à la demande (rAF coalescé), scène dynamique, `drawCounter`, obstacles schématiques `drawObstacle` (pastille `drawBadgeIcon` au coin du pion qui en occupe un), explosions `boom(hexes)` et glissements `slideUnit(unit, from)` auto-animés, surbrillances du tour IA (`ui.aiFocus` / `ui.aiTargets`)                                                                                                                                                             | `gfx`, `html`, `src/cards`, `src/sectors`, `src/movement` |
...
| `render/html.js`        | **PUR** : fragments HTML en chaînes (`cardHTML`, `tipHTML`, `calcHTML`, `forcePanelHTML`) + `SYM`, `UNIT_GLYPH`, `UNIT_ICON`, `faceHTML`, `SIDE_FR`                                                                                                                                                                                                                                                             | `src` (config, hex, sectors, movement, combat)            |
...
| `decks`      | `{ allies: [cardId], axis: [cardId] }` — répartition initiale 10/30                                                                                  |
...
| `ordersLeft` | ordres restants sur la carte jouée (total, borné par les unités activables)                                                                          |
| `orders`     | `{ secteur: ordres restants }` pour la carte en cours — quota décrémenté par `finishUnit`, filtre `activableUnits`                                   |
| `reconDraw`  | camp devant piocher 2 cartes / en garder 1 en fin de tour (carte `recon: true`), ou `null`                                                           |
| `reconChoice`| `{ side, ids }` — les 2 cartes piochées en attente de choix (`beginReconChoice` → `keepReconCard`), ou `null`                                        |
...
`breakthrough`, `action`, `cutWire`, `aiFocus`, `aiTargets`) vit dans
...
| `reconChoice`     | `game.js` `beginReconChoice` (via `endPlayerTurn` / `endAiTurn`)                                                                   | `{ side, ids }`                                                                                                             | `app.js` → picker joueur (`hand.showReconChoice`) ; le choix IA passe par `aiReconKeep` |
