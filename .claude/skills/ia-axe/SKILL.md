...
| Choix de carte          | `aiPickCard`           | `cardScore` (privée) : commandement = `min(activables, somme des quotas sectorQuota) × 2` (+3 par unité, dans cette limite, ayant déjà une cible via `targetsFor`) ; barrage = meilleure frappe `exp × 2` (+8 si létale) ; attaque aérienne = unités couvertes × 3 ; médecins = figurines perdues × 2 (−1 si personne) ; contre-attaque = score de la carte alliée rejouée (sinon Reconnaissance en force) |
...
| Choix des unités        | `aiChooseMoves`        | tri : peut frapper (+10), objectif à prendre à portée de mouvement (+5), − distance à l'ennemi le plus proche ; garde les meilleures dans la limite du quota de leur secteur (`sectorQuota`, hex à cheval : le secteur le mieux pourvu paie)                                                                                                                           |
