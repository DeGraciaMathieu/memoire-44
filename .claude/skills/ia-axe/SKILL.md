...
| ----------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Probabilité de touche   | `P_HIT` (const)        | par dé, selon la CIBLE — dérivée des règles (`hitOn` parmi `FACES`, × la relance `rerollHits` du Tigre) : inf 3/6, arm 2/6, art 3/6, tig 2/36 — un nouveau type est couvert d'office                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Plan d'une unité        | `aiPlanUnit`           | pour chaque destination (`sur place` + `reachable(moveRange)` — la carte tactique peut modifier ou interdire le mouvement) : tir éligible via `canFight` ; score tir = `espérance × 10` (+12 si létal, +10 si la cible occupe un objectif qui fait marquer le joueur) ; objectif : +12 sur une tuile qui rapporte à l'IA (prise ou maintien), sinon `(10 − dObj) × 0.5` vers l'objectif à prendre le plus proche (`openObjectives` : tuiles rapportant à l'IA qu'elle ne tient pas) ; protection : `(10 − dGuard) × 0.4` vers les tuiles réservées au joueur (`guardObjectives` — l'IA n'y marque jamais, elle en interdit l'accès) ; score position = `(10 − near) × 0.8` + `def × 1.5` ; artillerie −6 si `near < 3` |
| Prise de terrain        | `aiTakesGround`        | ne lâche jamais un objectif tenu ; avance toujours sur un objectif — même réservé au joueur : l'occuper en interdit l'accès ; sinon le blindé avance toujours (percée possible) et l'infanterie seulement si la couverture de l'hex pris (`defenseReduction` vs inf) est ≥ à celle de son hex                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
4. Nouveau type d'unité → `P_HIT` se dérive seul de `UNITS` (`hitOn`, `rerollHits`) ;
   rien à compléter tant que la vulnérabilité s'exprime par ces champs.
