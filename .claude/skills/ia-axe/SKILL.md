...
| Cible des médecins    | `aiMedicsTarget`       | l'unité de l'Axe la plus amochée ; à pertes égales, l'infanterie (meilleure espérance) ; `null` si personne                                                                                                                                                                                                                                                             |
...
1. `drawCards(state, 'axis')` → `aiPickCard` → `playCard(state, 'axis', id)` — qui
   renvoie la carte **effective** (Contre-attaque déjà résolue).
   Carte action → `playAxisAction` (cible via `aiBarrageTarget` / `aiAirHexes` /
